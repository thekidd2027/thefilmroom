...
function fmt(sec:number){const s=Math.max(0,Math.floor(sec||0));const m=Math.floor(s/60);return `${m}:${String(s%60).padStart(2,"0")}`}\nfunction ClipCard({clip,index}:{clip:ClipRef,index:number}){return <div className="bg-bay2 border border-rule rounded-[1rem] p-3 text-sm"><div className="flex justify-between gap-3"><div><span className="font-mono text-dim mr-2">{index}</span><span className="font-medium">{fmt(clip.start_seconds)}–{fmt(clip.end_seconds)}</span><span className="text-dim ml-2">{clip.moment||clip.story_function}</span></div><a target="_blank" href={clip.direct_url} className="btn-ghost shrink-0">Open @ {fmt(clip.start_seconds)}</a></div><div className="mt-1 text-dim">{clip.story_function}</div>{clip.rights_note&&<div className="mt-1 text-xs text-signal">Rights: {clip.rights_note}</div>}</div>}
